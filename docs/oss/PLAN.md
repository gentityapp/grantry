...
**（2026-09-18 追記・実行済み）** 運営者の指示（新しいリポジトリで公開する案の採用判断）で**枝 B を当日実行した**: grantry-mirror を main まで追従・履歴の再書き換え（ask ファイルに入った社内ドメインの役割アドレスのメールリテラル 2 コミット分を replace-text に追加・著者の一般ドメインのアドレス 10 コミットを mailmap で中立化・コミットメッセージ内の運営者実名 28 行と同じアドレスの trailer を --replace-message で除去・メール 0 氏名 0 トークン 0 を実測）・public 化・非公開脆弱性報告の有効化・good first issue 3 本（mirror #1〜#3）。`OSS_PUBLIC_REPO` の既定は mirror に変わった。本リポは private 残置で refs/pull（254 本）の除去依頼（asks 2026-09-17）は返信が来たら台帳に記録する。下の判断点 2026-09-24 は解消済み。
