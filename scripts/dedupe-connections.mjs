...
try {
  const connections = await prisma.connection.findMany({
    orderBy: [{ ownerId: "asc" }, { provider: "asc" }, { scope: "asc" }, { authType: "asc" }, { updatedAt: "desc" }],
  });

  const groups = new Map();
  for (const conn of connections) {
    const key = `${conn.ownerId}\u0000${conn.provider}\u0000${conn.scope}\u0000${conn.authType}`;
  ...
  for (const [key, group] of groups) {
    ...
    const [ownerId, provider, scope, authType] = key.split("\u0000");
