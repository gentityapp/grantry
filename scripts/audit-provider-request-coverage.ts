...
const EXPLICIT_NON_GENERIC_PROVIDERS: Record<string, string> = {
  ...
  microsoft_teams: "Microsoft Teams channel APIs need dedicated Microsoft Graph path construction and delegated message-body shaping",
  ...
  framer: "Framer is reached through the official Framer Server API session transport (framer-api connect/disconnect per call), not an HTTP API, so there is no provider/request passthrough",
