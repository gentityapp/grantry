...
export const PROVIDERS: Record<string, ProviderDef> = {
  ...
  clay: {
    ...
    tools: [
      "clay/me",
      "clay/search_reference",
      "clay/search",
      "clay/search_next",
      "clay/query_tables",
      "clay/run_routine",
      "clay/get_routine_run",
      "clay/query_workflow_runs",
      "clay/push_webhook",
      "clay/raw_request",
    ],
    implemented: true,
  ...
  x: {
    ...
    label: "X",
    authTypes: ["oauth"],
    helpText: "Create an OAuth 2.0 app in the X developer portal (Confidential client, type Web App) with the callback URL set to this app's /oauth/x/callback. Connect grants read access plus posting (with images), deleting tweets, and following / unfollowing accounts as the authorized account. Connections made before 2026-09-11 lack follows.write and must be re-authorized before follow_user works.",
    oauthSetupUrl: "https://developer.x.com/en/portal/dashboard",
    oauthClientAuthMethod: "CLIENT_SECRET_BASIC",
    oauthScopes: ["tweet.read", "tweet.write", "users.read", "offline.access", "media.write", "follows.read", "follows.write"],
    ...
    toolScopeRequirements: {
      ...
      "x/get_following": ["follows.read"],
      "x/follow_user": ["follows.write"],
      "x/unfollow_user": ["follows.write"],
    },
    implemented: true,
  },
  discord: {
  ...
  microsoft_teams: {
    ...
    authTypes: ["oauth"],
    helpText: "Connect a Microsoft identity platform OAuth app for Microsoft Graph. Register https://app.grantry.ai/oauth/microsoft_teams/callback as the redirect URI. Reads teams/channels/messages and sends channel messages as the signed-in work or school user. Channel message read permissions usually require Microsoft Entra admin consent.",
    tokenUrl: "https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationsListBlade",
    oauthSetupUrl: "https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationsListBlade",
    oauthAppOwner: "workspace",
    oauthScopes: ["offline_access", "User.Read", "Team.ReadBasic.All", "Channel.ReadBasic.All", "ChannelMessage.Read.All", "ChannelMessage.Send"],
    authorizeUrl: "https://login.microsoftonline.com/common/oauth2/v2.0/authorize",
    oauthTokenUrl: "https://login.microsoftonline.com/common/oauth2/v2.0/token",
    tools: [
      "microsoft_teams/get_me",
      "microsoft_teams/list_joined_teams",
      "microsoft_teams/list_channels",
      "microsoft_teams/list_messages",
      "microsoft_teams/get_message",
      "microsoft_teams/list_replies",
      "microsoft_teams/send_message",
      "microsoft_teams/send_reply",
    ],
    toolScopeRequirements: {
      "microsoft_teams/get_me": ["User.Read"],
      "microsoft_teams/list_joined_teams": ["Team.ReadBasic.All"],
      "microsoft_teams/list_channels": ["Channel.ReadBasic.All"],
      "microsoft_teams/list_messages": ["ChannelMessage.Read.All"],
      "microsoft_teams/get_message": ["ChannelMessage.Read.All"],
      "microsoft_teams/list_replies": ["ChannelMessage.Read.All"],
      "microsoft_teams/send_message": ["ChannelMessage.Send"],
      "microsoft_teams/send_reply": ["ChannelMessage.Send"],
    },
    implemented: true,
  },
  aws: {
  ...
  framer: {
    ...
    label: "Framer",
    authTypes: ["pat"],
    helpText: "Framer API keys are issued per project: open the project in Framer, open Settings (Cmd+K -> \"open settings\"), find API Keys, and create a key. Paste it together with the project as JSON {\"project\":\"https://framer.com/projects/Site--abc123\",\"api_key\":\"...\"} (or \"<project URL or id>|<key>\"). Grantry connects through the official Framer Server API for each call. apply_changes writes canvas commands to one page; read get_agent_system_prompt first for the command syntax. publish_preview only publishes a branch preview: production publishing and arbitrary code execution are not exposed. Custom domains, form destinations and form redirects are not available in the Framer API and stay manual in the Framer UI.",
    tokenUrl: "https://www.framer.com/developers/server-api-quick-start",
    tools: [
      "framer/get_project_info",
      "framer/get_agent_system_prompt",
      "framer/get_agent_context",
      "framer/list_branches",
      "framer/create_branch",
      "framer/switch_branch",
      "framer/read_project",
      "framer/apply_changes",
      "framer/publish_preview",
    ],
    implemented: true,
  },
  apify: {
...
const GENERIC_REQUESTS: Record<string, NonNullable<ProviderDef["genericRequest"]>> = {
  ...
  clay: {
    ...
    defaultMethods: ["GET"],
    allowedPathPrefixes: ["/public/v0/"],
    authScheme: "api_key",
    apiKeyHeader: "clay-api-key",
    smokeTests: [{ id: "me", method: "GET", path: "/public/v0/me" }],
    operations: [
      { id: "me", tools: ["clay/me"], method: "GET", path: "/public/v0/me", description: "Authenticated user and workspace for the API key.", risk: "read", probeQuery: {} },
