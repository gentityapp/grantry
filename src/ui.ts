            ...
            <tbody>
              ${catalog.map(({ provider: p, enabled, pinned, explicit }) => {
                ...
                <tr class="provider-catalog-row" data-search="${escapeHtml((p.key + " " + p.label + " " + p.authTypes.join(" ")).toLowerCase())}">
                  <td><span class="provider-cell">${providerIcon(p.key)}<span>${escapeHtml(p.label)}</span></span><br><span style="color:#687385;font-size:12px;">key: <code>${escapeHtml(p.key)}</code></span></td>
